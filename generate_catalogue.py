...
UI = r"""
(function(){
const PHONE='919465263877',EMAIL='kunalkhurana@makoverseas.com',products=window.MAK_PRODUCTS||[],categories=window.MAK_CATEGORIES||[],$=(s,r=document)=>r.querySelector(s),$$=(s,r=document)=>Array.from(r.querySelectorAll(s));
...
def write_pages():
    ...
    for slug, title, page, image, summary in CATEGORIES:
        main = f"""<div class="toolbar"><div><strong>{html.escape(title)}</strong><br><small id="itemCount">Loading products</small></div><input id="filterInput" type="search" placeholder="Search within {html.escape(title)} by name, code or part number..."></div><section class="grid" id="productGrid" data-category="{html.escape(slug)}"></section><div class="notice">Every item can be added to the enquiry cart or sent directly by WhatsApp or email with its product name and part number.</div>"""
